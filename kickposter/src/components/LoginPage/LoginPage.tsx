...
const LoginPage: React.FC<LoginProps> = (props: LoginProps) => {
  ...
  function rateLimit(username: string) {
    ...
    setTimesSinceLastAttempt(
      new Map(timesSinceLastAttempt.set(username, timeAtAttempt))
    );
    return (
      lastAttemptTime !== undefined &&
      timeAtAttempt - lastAttemptTime < minSlowdown
    );
  }
  ...
  function handleFormSubmit(event: { preventDefault: () => void }) {
    ...
    setUsernameError("");
    setPasswordError("");
    if (usernameInput && rateLimit(usernameInput)) {
      setUsernameError("You need to wait longer before trying again!");
      return;
    }
